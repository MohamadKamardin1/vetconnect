    ...
    <section className="flex items-center justify-center p-5 sm:p-10 lg:min-h-0 lg:p-6 xl:p-8">
      <div className="w-full max-w-md lg:max-w-[31rem]">
        ...
        <form className={`mt-8 grid gap-4 lg:mt-4 lg:gap-x-3 lg:gap-y-2.5 ${!isLogin ? "lg:grid-cols-2" : ""}`} onSubmit={submit}>
          ...
          <button disabled={state === "loading"} className={`ink-button mt-2 disabled:opacity-60 lg:mt-0 lg:py-2.5 ${!isLogin ? "lg:col-span-2" : ""}`}>{state === "loading" ? t(isLogin ? "Signing in…" : "Creating account…") : t(isLogin ? "Sign in securely" : "Continue to profile studio")}<ArrowRight size={16} /></button>
        </form>
