...
export function BrandMark({ compact = false, tone = "light" }: { compact?: boolean; tone?: "light" | "dark" }) {
  return <Link href="/" className="flex items-center gap-3" aria-label="VetKonnect home">
    ...
    {!compact && <span className="leading-none"><span className={`block text-lg font-extrabold tracking-[-0.075em] ${tone === "dark" ? "text-[#fffaf0]" : "text-[#244c3d]"}`}>VetKonnect</span><span className="mt-1 block text-[8px] font-extrabold uppercase tracking-[0.19em] text-[#cda95b]">Tanzania & Zanzibar</span></span>}
  </Link>;
}
